...
export class TransactionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.service.totalMerchantMachineTransactions(this.commonData.merchantId).subscribe((resp: any) => {
  ...
  getTransactions() {
    if(this.totalRows > 0) {
      this.service.getMerchantMachineTransactions(this.commonData.merchantId, 15, this.currentPage).subscribe((resp: any) => {
        console.log("getMerchantTransactions Reponse:::" + JSON.stringify(resp));
        if (resp) {
          console.log("Txns...............................")
          this.tableData = resp;
          this.returnedArray = this.tableData;
        }
      });
    }
  }
