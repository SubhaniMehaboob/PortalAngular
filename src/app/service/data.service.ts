...
export class DataService {
 ...
 httpOptions = {
  ...
  saveMachineStatus(machineStatus): Observable<any> {
    console.log("MachineStatus..."+this.url + '/updateMen')
    return this.http.post(this.url + '/updateMen', JSON.stringify(machineStatus), this.httpOptions)
    .pipe(
      retry(1),
      catchError(this.handleError)
    )
  ...
  getMachines(merchantId: string,fromno:number,count:number): Observable<any> {
    console.log("Connecting..."+this.url + '/machines/'+merchantId+'/'+fromno+'/'+count)
    return this.http.get(this.url + '/machines/'+merchantId+'/'+fromno+'/'+count, this.httpOptions)
    .pipe(
      retry(1),
      catchError(this.handleError)
    )
